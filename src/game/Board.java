...
public class Board {
    ...
    public void addRandomTile() {
        int amountOfOpenSpots = openSpaces.size();
        int randomBoardSpot = StdRandom.uniform(0, amountOfOpenSpots);
        BoardSpot tile = openSpaces.get(randomBoardSpot);
        ...
        gameBoard[tile.getRow()][tile.getCol()] = value;

        openSpaces.remove(randomBoardSpot);
    }
    ...
    public void flipRows() {
        ...
        for (int i =0; i < rows; i++){
            int start = 0;
            int end = gameBoard[i].length-1;
            while (start < end){
                int temp = gameBoard[i][start];
                gameBoard[i][start] = gameBoard[i][end];
                gameBoard[i][end] = temp;
                start++;
                end--;
            }
        }
